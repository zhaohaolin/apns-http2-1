...
class HttpClientHandler<T extends HttpPushNotification> extends Http2ConnectionHandler {
    ...
    public static class ApnsClientHandlerBuilder<S extends HttpPushNotification> extends AbstractHttp2ConnectionHandlerBuilder<HttpClientHandler<S>, ApnsClientHandlerBuilder<S>> {

        private PushHttpClient<S> pushHttpClient;
        private String authority;
        private int maxUnflushedNotifications = 0;

        public ApnsClientHandlerBuilder<S> apnsClient(final PushHttpClient<S> pushHttpClient) {
        ...
        @Override
        public HttpClientHandler<S> build(final Http2ConnectionDecoder decoder, final Http2ConnectionEncoder encoder, final Http2Settings initialSettings) {
            Objects.requireNonNull(this.authority(), "Authority must be set before building an HttpClientHandler.");

            final HttpClientHandler<S> handler = new HttpClientHandler<>(decoder, encoder, initialSettings, this.apnsClient(), this.authority(), this.maxUnflushedNotifications());
    ...
    private class ApnsClientHandlerFrameAdapter extends Http2FrameAdapter {
        ...
        public int onDataRead(final ChannelHandlerContext context, final int streamId, final ByteBuf data, final int padding, final boolean endOfStream) throws Http2Exception {
            ...
            if (endOfStream) {
                final Http2Headers headers = HttpClientHandler.this.headersByStreamId.remove(streamId);
                final T pushNotification = HttpClientHandler.this.pushNotificationsByStreamId.remove(streamId);
                ...
                HttpClientHandler.this.pushHttpClient.handlePushNotificationResponse(new ZupPushNotificationResponse<>(
        ...
        public void onGoAwayRead(final ChannelHandlerContext context, final int lastStreamId, final long errorCode, final ByteBuf debugData) throws Http2Exception {
            ...
            HttpClientHandler.this.pushHttpClient.abortConnection(errorResponse);
        }
    }

    protected HttpClientHandler(final Http2ConnectionDecoder decoder, final Http2ConnectionEncoder encoder, final Http2Settings initialSettings, final PushHttpClient<T> pushHttpClient, final String authority, final int maxUnflushedNotifications) {
        ...
        try {
            ...
            writePromise.addListener(new GenericFutureListener<ChannelPromise>() {
                ...
                public void operationComplete(final ChannelPromise future) throws Exception {
                    if (future.isSuccess()) {
                        HttpClientHandler.this.pushNotificationsByStreamId.put(streamId, pushNotification);
                    } else {
            ...
            if (IdleState.WRITER_IDLE.equals(idleStateEvent.state())) {
                ...
                this.encoder().writePing(context, false, pingDataBuffer, context.newPromise()).addListener(new GenericFutureListener<ChannelFuture>() {
                    ...
                    public void operationComplete(final ChannelFuture future) throws Exception {
                        if (future.isSuccess()) {
                            HttpClientHandler.this.pingTimeoutFuture = future.channel().eventLoop().schedule(new Runnable() {
