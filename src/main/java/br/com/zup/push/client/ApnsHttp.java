...
public class ApnsHttp {
	...
	public ApnsHttp(final InputStream p12InputStream, final String password)
			throws SSLException {
		try {
	...
	public PushNotificationResponse<ZupHttpPushNotification> pushMessageSync(
			final String message, final String token)
			throws ExecutionException, CertificateNotValidException {
		try {
			...
			{
				pushNotification = new ZupHttpPushNotification(token, null,
						message);
			}
			final Future<PushNotificationResponse<ZupHttpPushNotification>> sendNotificationFuture = this.httpClient
					.sendNotification(pushNotification);
			final PushNotificationResponse<ZupHttpPushNotification> pushNotificationResponse = sendNotificationFuture
					.get();
	 ...
	 */
	public Future<PushNotificationResponse<ZupHttpPushNotification>> pushMessageAsync(
			final String message, final String token) throws ExecutionException {
		if (!this.httpClient.isConnected()) {
			try {
				stablishConnection();
			} catch (InterruptedException e) {
